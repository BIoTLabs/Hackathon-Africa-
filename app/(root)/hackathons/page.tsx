...
export default async function AllHackathonsPage() {
  ...
  return (
    <>
      ...
      <div className='pb-20 bg-[#FAFAFA] md:px-20 px-5 xxl:px-96 py-10 lg:py-24 mt-0.5'>
          ...
          {hackathons.map((hackathon: any) => (
            ...
            >
              <div className='max-w-md bg-white border border-gray-300 rounded-lg shadow-sm relative'>
                ...
                <div className='p-5'>
                  <div className='flex space-x-2 items-center text-sm text-[#FF761C]'>
                    <BsClock className='h-5 w-5' />
                    <p>{hackathon.type}</p>
                    <p>{hackathon.daysLeft} left</p>
                  </div>
                  <h3 className='text-base font-semibold line-clamp-1 mt-3'>
                    {hackathon.title}
                  </h3>
                  <p className='mb-2 text-[14px] font-normal text-gray-500 line-clamp-3'>
                    {hackathon.description}
                  </p>
                  <h2 className='text-[12px] line-clamp-3'>{hackathon.text}</h2>
                  <p className='text-[#FF761C] font-bold text-lg mt-2'>
